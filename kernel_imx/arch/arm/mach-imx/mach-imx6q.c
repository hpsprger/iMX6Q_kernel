...
static int ar8035_phy_fixup(struct phy_device *dev)
{
	 ...
	 */	
	val = 0;
	val = phy_read(dev, 0xd);
	//printk("++++[%s +++ %s- %d] phy_read 0xd = %d \n",__FILE__,__func__,__LINE__,val);	
	phy_write(dev, 0xd, 0x3);
	val = 0;
	val = phy_read(dev, 0xd);
	//printk("++++[%s +++ %s- %d] phy_read 0xd = %d \n",__FILE__,__func__,__LINE__,val);	
...
#define PHY_ID_RTL8211B 0x001cc912
#define PHY_ID_RTL8211E 0x001cc915

static int ar8221_phy_fixup(struct phy_device *dev)
{
	u16 val = 0;
	//printk("+++[%s: %s - %d]	ar8221_phy_fixup \n",__FILE__,__func__,__LINE__);
	/* ar8221 phy SmartEEE feature cause link status generates glitch,
	 * which cause ethernet link down/up issue, so disable SmartEEE
	 */
	 val = phy_read(dev, 0xd);
	//printk("++++[%s +++ %s- %d] phy_read 0xd = %d \n",__FILE__,__func__,__LINE__,val);	
...
//#ifdef CONFIG_REALTEK_PHY		
		phy_register_fixup_for_uid(PHY_ID_RTL8211B, 0x001fffff,
				ar8221_phy_fixup);
		phy_register_fixup_for_uid(PHY_ID_RTL8211E, 0x001fffff,
				ar8221_phy_fixup);
//#else		
