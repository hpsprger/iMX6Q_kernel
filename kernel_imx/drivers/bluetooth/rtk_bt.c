...
static void btusb_intr_complete(struct urb *urb)
{
	...
	if (urb->status == 0) {
	    ...
	    pp = urb->transfer_buffer;
        ev_code = *pp;
        
        if(ev_code == HCI_EV_CMD_COMPLETE) {
            pp += 3;
		    STREAM_TO_UINT16(cmd_code, pp);
            //RTKBT_DBG("cmd_complete, cmd_code is 0x%x, lase_send_cmd is 0x%x", cmd_code, usb_coex_info.last_send_cmd);
            if(cmd_code == usb_coex_info.last_send_cmd)
                need_sendto_core = 0;
        }
        ...
        if(ev_code == HCI_EV_CMD_STATUS) {
            pp += 4;
		    STREAM_TO_UINT16(cmd_code, pp);
            //RTKBT_DBG("cmd_status, cmd_code is 0x%x, last_send_cmd is 0x%x", cmd_code, usb_coex_info.last_send_cmd);
            if(cmd_code == usb_coex_info.last_send_cmd)
                need_sendto_core = 0;
        }
...
int rtk_parse_config_file(unsigned char* config_buf, int* filelen, char bt_addr[6])
{
    ...
    if (config_len != *filelen - sizeof(struct rtk_bt_vendor_config)) {
        RTKBT_ERR("config len(%d) is not right(%zd)", config_len, *filelen-sizeof(struct rtk_bt_vendor_config));
        return 0;
    }
...
uint8_t rtk_get_fw_project_id(uint8_t *p_buf)
{
    ...
    do {
        ...
        if (opcode == 0x00) {
            if (len == 1) {
                data = *(p_buf - 2);
                RTKBT_DBG("rtk_get_fw_project_id: opcode %d, len %d, data %d", opcode, len, data);
                break;
            } else {
                RTKBT_ERR("rtk_get_fw_project_id: invalid len %d", len);
            }
        }
        p_buf -= len + 2;
    } while (*p_buf != 0xFF);
...
static void rtk_get_patch_entry(uint8_t *epatch_buf, struct rtk_epatch_entry *entry)
{
    uint16_t i;
    ...
    {
        if(get_unaligned_le16(epatch_buf+14+2*i) == gEVersion + 1)
        {
            entry->chipID = gEVersion + 1;
            entry->patch_length = get_unaligned_le16(epatch_buf+14+2*epatch_info->number_of_total_patch+2*i);
            entry->start_offset = get_unaligned_le32(epatch_buf+14+4*epatch_info->number_of_total_patch+4*i);
            break;
        }
    }
...
int load_firmware(dev_data* dev_entry, uint8_t** buff)
{
	...
	if(lmp_version == ROM_LMP_8723a) {
		...
		if((memcmp(epatch_buf, RTK_EPATCH_SIGNATURE, 8) != 0) ||
            memcmp(epatch_buf+buf_len-config_len-4, Extension_Section_SIGNATURE, 4) != 0) {
			...
			if(lmp_version != project_id[proj_id]) {
				RTKBT_ERR("lmp_version is %x, project_id is %x, does not match!!!", lmp_version, project_id[proj_id]);
				need_download_fw = 0;
			} else {
				RTKBT_DBG("lmp_version is %x, project_id is %x, match!", lmp_version, project_id[proj_id]);
	...
	RTKBT_DBG("fw:%s exists, config file:%s exists", (buf_len > 0) ? "":"not", (config_len>0)?"":"not");
...
uint8_t rtk_get_eversion(dev_data * dev_entry)
{
	...
	RTKBT_DBG("eversion->status = 0x%x, eversion->version = 0x%x",eversion->status, eversion->version);
	if (eversion->status) {
		ret_val = 0;
	    //global_eversion = 0;
	}
