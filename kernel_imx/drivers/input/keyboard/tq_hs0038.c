...
/*tq_hs0038_keycode必须包含所有要用到的系统键值*/
static const unsigned int tq_hs0038_keycode[]={
...
static const unsigned int tq_hs0038_keycode_807F[]={
	0,/*NULL*/0,/*NULL*//*0~1*/
...
static const unsigned int tq_hs0038_keycode_00FF[32]={
...
static const unsigned int tq_keycode_00FF[32]={
	0x10,0x03,0x01,0x06,
	0x09,0x1d,0x1f,0x0d,
	0x19,0x1b,0x11,0x15,
	0x17,0x12,0x16,0x4d,
...
struct tq_hs0038
{
	unsigned int keycode[ARRAY_SIZE(tq_hs0038_keycode)];
...
{
	int i;
	for(i=0 ; i<sizeof(tq_keycode_00FF)/sizeof(tq_keycode_00FF[0]) ; i++)
	{
		if(code == tq_keycode_00FF[i])
		{
			if(sizeof(tq_hs0038_keycode_00FF)/sizeof(tq_hs0038_keycode_00FF[0]) >= i)
			return  tq_hs0038_keycode_00FF[i];
		}
	}
	return 0xFFFF;
}
...
static u32 __inline__ serial_data_read_byte(struct tq_hs0038 *hs0038_data)
{
...
static int tq_hs0038_read(struct tq_hs0038 *hs0038_data)
{
	u32 values[4];
	int i=0;
	struct gpio_keys_button *button = hs0038_data->button;
...
/*ccq for(i=0;i<sizeof(values)/sizeof(values[0]);i++)
	{
	...
	{
		i=values[2];
		input_report_key(hs0038_data->input,tq_hs0038_keycode_807F[i], 1);
		input_report_key(hs0038_data->input,tq_hs0038_keycode_807F[i], 0);
		input_sync(hs0038_data->input);
...
void hs0038_do_work(struct work_struct *work)
{
	struct tq_hs0038 *hs0038;
	hs0038 = container_of(work, struct tq_hs0038, hs0038_wp);
	...
	tq_hs0038_read(hs0038);
	irq_set_irq_type(irq,  IRQF_TRIGGER_FALLING);
	enable_irq(irq);
}

static irqreturn_t tq_hs0038_interrupt(int irq, void *dev_id)
{
	struct tq_hs0038 *hs0038 = dev_id;
...
{
	struct tq_hs0038 *hs0038 = platform_get_drvdata(pdev);
	struct gpio_keys_button *button=hs0038->button;
	...
	error = request_irq(irq, tq_hs0038_interrupt, irqflags, desc, hs0038);
	if (error) {
...
static int tq_hs0038_probe(struct platform_device *pdev)
{
	int ret = 0;
	...
	hs0038= kzalloc(sizeof(struct tq_hs0038),GFP_KERNEL);
	...
	INIT_WORK(&hs0038->hs0038_wp, hs0038_do_work);   //dc
	...
	input->name = "tq_hs0038";
	input->phys = "tq_hs0038/input1";
	...
	input->keycodemax = ARRAY_SIZE(tq_hs0038_keycode);
	set_bit(EV_KEY, input->evbit);


	for (i = 0; i < ARRAY_SIZE(tq_hs0038_keycode); i++){
		hs0038->keycode[i]= tq_hs0038_keycode[i];
	...
	if (ret < 0){
		printk(KERN_ERR "tq_hs0038.c: input_register_device() return %d!\n", ret);
		goto  err_input_free;
	}
...
static int tq_hs0038_remove(struct platform_device *pdev)
{
	struct tq_hs0038 *s3c_hs0038 = platform_get_drvdata(pdev);
	struct gpio_keys_platform_data *pdata = pdev->dev.platform_data;
...
#ifdef CONFIG_PM
static int tq_hs0038_suspend(struct device *dev)
{
...
static int tq_hs0038_resume(struct device *dev)
{
...
static const struct dev_pm_ops tq_hs0038_pm_ops = {
	.suspend	= tq_hs0038_suspend,
	.resume		= tq_hs0038_resume,
};
#else
#define  tq_hs0038_suspend NULL
#define  tq_hs0038_resume NULL
#endif
...
static struct platform_driver s3c_hs0038_driver = {
	.probe		= tq_hs0038_probe,
//	.remove		= __devexit_p(tq_hs0038_remove),
	.driver		= {
		.name   = "hs0038",
		.owner	= THIS_MODULE,
#ifdef CONFIG_PM
...
static int __init tq_hs0038_init(void)
{
	return platform_driver_register(&s3c_hs0038_driver);
}

static void __exit tq_hs0038_exit(void)
{
	platform_driver_unregister(&s3c_hs0038_driver);
}

module_init(tq_hs0038_init);
module_exit(tq_hs0038_exit);

MODULE_AUTHOR("www.embedsky.net");
MODULE_DESCRIPTION("tq GPIO Keyboard Driver");
MODULE_LICENSE("GPL");
