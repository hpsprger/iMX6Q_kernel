...
#ifdef CONFIG_DRM
	ret = msm_drm_register_client(&ilitek_data->fb_notif);	//tanya
#else
...
{
	printk(KERN_ERR,"xxxxxxxxxxxxxxxxxxxxxx\n");	//tanya
	int ret = 0;
//#if ILITEK_PLAT != ILITEK_PLAT_MTK
...
#if defined(CONFIG_FB) || defined(CONFIG_DRM)
#ifdef CONFIG_DRM
		msm_drm_unregister_client(&ilitek_data->fb_notif);	//tanya
#else
