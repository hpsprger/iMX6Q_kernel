...
static int32_t ilitek_sensortest_lego_series(int32_t *short_xdata1, int32_t *short_ydata1, int32_t *open_data, int32_t *allnode_data, int32_t *allnode_win1, int32_t *allnode_win2) {
	int32_t ret = ILITEK_SUCCESS;
	...
	if(ilitek_data->mp.open) {
		...
		ret = ilitek_open_test_v6(open_data);
		if (ret < 0) {
...
static int32_t ilitek_sensortest_proc_show(struct seq_file *m, void *v)
{
	...
	|| NULL == allnode_data || NULL == allnode_win1 || NULL == allnode_win2) {
	...
	if(ilitek_data->ptl.ver_major == 0x3) {
		...
		ret = ilitek_sensortest_lego_series(short_xdata1, short_ydata1, open_data, allnode_data, allnode_win1, allnode_win2);
	}
	else {
	...
	if(ilitek_data->ptl.ver_major == 0x3) {
		...
		ilitek_printf_sensortest_data_v6(short_xdata1, short_ydata1, open_data, allnode_data, allnode_win1, allnode_win2, m);
	}
	else {
		tp_log_err("The is protocol not support, protocol:0x%06x\n", ilitek_data->ptl.ver);
	}	
