...
static uint32_t findstr(uint8_t *data, int n, const uint8_t *str, int len)
{
	int i, j;
...
uint32_t get_ap_endaddr_3XX(uint32_t startAddr,uint32_t endAddr, uint8_t input[]) {
	uint32_t addr;
	...
	addr = findstr(input, endAddr, tag, 32) - 1 + 32; // 32 is Tag
...
int32_t ilitek_bin_upgrade_3XX(struct device *dev, const char *fn) {
	...
	df_startaddr = (buf[2] << 16)  + (buf[3] << 8) + buf[4];
	ap_endaddr = get_ap_endaddr_3XX(ap_startaddr, df_startaddr,CTPM_FW + ILITEK_ILI_HEADER_LENGTH);
