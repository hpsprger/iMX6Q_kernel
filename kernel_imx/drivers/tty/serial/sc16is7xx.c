...
static u8 sc16is7xx_port_raw_read(struct uart_port *port, u8 reg, u8 *buf, int rxlen)
{
	...
	ret = regmap_raw_read(s->regmap, (reg << SC16IS7XX_REG_SHIFT) | (port->line<<1) ,
			buf, rxlen);
...
static u8 sc16is7xx_port_raw_write(struct uart_port *port, u8 reg, u8 *buf, int to_send)
{
	struct sc16is7xx_port *s = dev_get_drvdata(port->dev);
	unsigned int val = 0;
	int ret;
	...
	regcache_cache_bypass(s->regmap, true);
	ret = regmap_raw_write(s->regmap, SC16IS7XX_THR_REG | (port->line<<1), buf, to_send);
	regcache_cache_bypass(s->regmap, false);
...
static u8 sc16is7xx_port_read(struct uart_port *port, u8 reg)
{
	...
	regmap_read(s->regmap,
		    (reg << SC16IS7XX_REG_SHIFT) | (port->line<<1), &val);
				  ...
				  u8 mask, u8 val)
{
	...
	regmap_update_bits(s->regmap,
			   (reg << SC16IS7XX_REG_SHIFT) | (port->line<<1),
			   mask, val);
}
				...
				unsigned int iir)
{
	...
	while (rxlen) {
		...
		if (read_lsr) {
			...
			regcache_cache_bypass(s->regmap, false);
#else 
			sc16is7xx_port_raw_read(port, SC16IS7XX_RHR_REG, s->buf, rxlen);
#endif			
			bytes_read = rxlen;
		}
...
static void sc16is7xx_handle_tx(struct uart_port *port)
{
	...
	//	log_d("uart_circ_empty==%d || uart_tx_stopped=%d\n", uart_circ_empty(xmit), uart_tx_stopped(port));
		return;
	}
...
//			log_d("send[%d]==%c\n", i, s->buf[i]);
		...
		regcache_cache_bypass(s->regmap, false);
#else 
		sc16is7xx_port_raw_write(port, SC16IS7XX_THR_REG, s->buf, to_send);
#endif		
	}
...
//		log_d("iir src = %x, SC16IS7XX_IIR_RDI_SRC = %x SC16IS7XX_IIR_THRI_SRC = %x\n", \
			iir, SC16IS7XX_IIR_RDI_SRC, SC16IS7XX_IIR_THRI_SRC);
