...
static enum crtc of_get_crtc_mapping(struct device_node *np)
{
	...
	for (i = 0; i < ARRAY_SIZE(ldb_crtc_mappings); i++)
		if (!strcasecmp(cm, ldb_crtc_mappings[i])) {
			...
			if(fb_ipu_di("lvds", &ipu_id, &disp_id)) {
				if((ipu_id == 0) && (ipu_id == 0)) {
					i = CRTC_IPU1_DI0;
				} else if((ipu_id == 0) && (ipu_id == 1)) {
					i = CRTC_IPU1_DI1;
				} else if((ipu_id == 1) && (ipu_id == 0)) {
					i = CRTC_IPU2_DI0;
				} else if((ipu_id == 1) && (ipu_id == 1)) {
					i = CRTC_IPU2_DI1;
				}
			}
			return i;
...
static int ldb_probe(struct platform_device *pdev)
{
	...
	for_each_child_of_node(np, child) {
		...
		ret = of_property_read_u32(child, "fsl,data-width",
					   &data_width);
		struct tq_ldb mode;
		mode.dual_mode = ldb->dual_mode;
		mode.spl_mode = ldb->spl_mode;
		mode.data_width = data_width;
		if(fb_replace_modedbs("ldb_data",&mode)) {
			ldb->dual_mode = mode.dual_mode;
			ldb->spl_mode = mode.spl_mode;
			data_width = mode.data_width;
		}
